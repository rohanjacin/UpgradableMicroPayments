...
contract paymenChannel {
    ...
    function withdrawChannel(address payer, bytes32 finalHashValue, uint256 numberOfTokensUsed) public {
        Channel storage channel = channelsMapping[payer][msg.sender]; // Use storage to update state directly
        require(channel.amount > 0, "Channel does not exist or has been withdrawn.");
        
        require(utility.verifyHashchain(channel.trustAnchor, finalHashValue, numberOfTokensUsed), "Verification failed.");

        uint256 payableAmount = (channel.amount * numberOfTokensUsed) / channel.numberOfTokens;
